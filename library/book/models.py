...
class Book(models.Model):
    ...
    count = models.IntegerField(default=10, verbose_name='Copies available')
    id = models.AutoField(primary_key=True, verbose_name='Id')
    author = models.ForeignKey('author.Author', on_delete=models.CASCADE, verbose_name='Author')
    publication_year = models.PositiveIntegerField(verbose_name='Publication year', default=None)
    issue_date = models.DateField(verbose_name='Date of issue', default=datetime.date.today)
    ...
    def __str__(self):
        ...
        """
        # return f"'id': {self.id}, 'name': '{self.name}', 'description': '{self.description}', 'count': {self.count}, 'author': {self.author}"
        return f"{self.name}"
